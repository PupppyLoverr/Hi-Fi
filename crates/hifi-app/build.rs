...
/// Linux pages render in a WebKitGTK helper process; compile it here so the
/// app can embed it (see `src/webview/linux`).
fn main() {
    println!("cargo:rerun-if-changed=src/webview/linux/helper.c");
    if env::var("CARGO_CFG_TARGET_OS").as_deref() != Ok("linux") {
        return;
    }
    let output = PathBuf::from(env::var_os("OUT_DIR").unwrap()).join("hifi-webkit");
    ...
    assert!(
        ...
        "The Linux browser requires WebKitGTK 4.1 and JSON-GLib development files \
         (webkit2gtk-4.1 and json-glib-1.0 via pkg-config).\n{}",
    ...
    let status = Command::new(env::var("CC").unwrap_or_else(|_| "cc".into()))
        .args([
            ...
            "src/webview/linux/helper.c",
            "-o",
        ])
        .arg(&output)
        .args(String::from_utf8(flags.stdout).unwrap().split_whitespace())
        .status()
        .expect("a C compiler is required to build the Linux browser helper");
