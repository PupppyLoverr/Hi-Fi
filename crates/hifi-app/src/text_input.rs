//! `TextField` — single-line text input, adapted from zed's gpui `input`
//! example (ElementInputHandler pattern) with submit/change callbacks.
...
/// Subscribe for text/submit events.
...
pub struct TextField {
    ...
    is_selecting: bool,
    /// Text color applied to content runs (placeholder is faint always).
    pub text_color: Option<gpui::Hsla>,
    pub placeholder_color: Option<gpui::Hsla>,
}
...
impl TextField {
    ...
    fn submit(&mut self, _: &Submit, _window: &mut Window, cx: &mut Context<Self>) {
        cx.emit(TextFieldEvent::Submitted(self.content.to_string()));
    }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            ...
            (content, input.text_color.unwrap_or(style.color))
        };
        ...
        let accent = input.text_color.unwrap_or(gpui::blue());
