// Ported from cosmos. WebKitGTK runs in its own process. Only rendered pixels and explicit browser
...
static void ipc_message(WebKitUserContentManager *ucm, WebKitJavascriptResult *result, Page *p) {
    gchar *text = jsc_value_to_string(webkit_javascript_result_get_js_value(result));
    send_packet('P', p->id, text, strlen(text));
    g_free(text);
}
static Page *new_page(guint id, gboolean ipc) {
    ...
    if (ipc) {
        // Editor surfaces persist through `window.ipc.postMessage`.
        g_signal_connect(ucm, "script-message-received::ipc", G_CALLBACK(ipc_message), p);
    ...
    p->web = WEBKIT_WEB_VIEW(g_object_new(WEBKIT_TYPE_WEB_VIEW, "web-context", context,
...
static void command(JsonObject *o) {
    ...
    if (!strcmp(cmd, "create")) {
        if (!p)
            new_page(id, json_object_has_member(o, "ipc") && json_object_get_boolean_member(o, "ipc"));
        return;
    }
    if (!p)
        return;
    ...
    if (!strcmp(cmd, "load")) {
        ...
        webkit_web_view_load_html(p->web, string(o, "html"), "about:blank");
